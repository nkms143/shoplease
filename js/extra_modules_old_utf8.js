...
const SettingsModule = {
    render(container) {
        ...
        container.innerHTML = `
            <div class="glass-panel" style="max-width: 600px; margin: 0 auto;">
                <h3 style="margin-bottom: 2rem; border-bottom: 2px solid #e2e8f0; padding-bottom: 0.5rem;">ΓÜÖ∩╕Å Application Settings</h3>
                ...
                <form id="settings-form">
                    ...
                    <div style="background: #fdf2f8; padding: 1.5rem; border-radius: 8px; margin-bottom: 2rem; border: 1px solid #fbcfe8;">
                         <h4 style="margin-top: 0; color: #be185d; margin-bottom: 1rem;">≡ƒ¢æ Penalty Rate History</h4>
                         <p style="font-size: 0.9rem; color: #64748b; margin-bottom: 1rem;">
                            Define penalty rates over time. The system picks the rate based on the effective date.
                            Default: Γé╣500/Month (from 2023-01-01).
                         </p>
                         ...
                         <div id="active-penalty-policy" style="background: white; padding: 1rem; border-radius: 8px; border: 2px dashed #fbcfe8; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 1rem;">
                            <div style="font-size: 1.5rem;">≡ƒôó</div>
                            <div>
                             ...
                             <div style="display: flex; gap: 0.5rem; align-items: end; flex-wrap: wrap;">
                                <div>
                                 ...
                                 <div style="flex: 1; min-width: 120px;">
                                    <label class="form-label">Rate (Γé╣)</label>
                    ...
                    <div style="background: #eff6ff; padding: 1.5rem; border-radius: 8px; margin-bottom: 2rem; border: 1px solid #dbeafe;">
                         <h4 style="margin-top: 0; color: #1d4ed8; margin-bottom: 1rem;">≡ƒº╛ GST Rate History</h4>
                         <p style="font-size: 0.9rem; color: #64748b; margin-bottom: 1rem;">
                    ...
                    <div style="background: #fff; padding: 1.5rem; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 2rem;">
                        <h4 style="margin-top: 0; color: #334155; margin-bottom: 1rem;">≡ƒû╝∩╕Å Logo Settings</h4>
                    ...
                    <button type="submit" class="btn-primary" style="width: 100%; padding: 1rem; font-size: 1.1rem;">≡ƒÆ╛ Save Settings</button>
                </form>

                <!-- Data Management -->
                <div style="margin-top: 3rem; border-top: 2px solid #e2e8f0; padding-top: 2rem;">
                    <h3 style="margin-bottom: 1.5rem;">≡ƒÆ╛ Data Management</h3>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                        <button id="btn-backup" class="btn-primary" style="background: #3b82f6;">Γ¼ç∩╕Å Download Backup</button>
                        <button id="btn-restore" class="btn-primary" style="background: #8b5cf6;">Γ¼å∩╕Å Restore Backup</button>
                        <button id="btn-cloud-backup" class="btn-primary" style="background: #0ea5e9;">Γÿü∩╕Å Cloud Sync</button>
                    </div>
    ...
    renderPenaltyList() {
        ...
        if (activeTextPlaceholder) {
            ...
            activeTextPlaceholder.innerHTML = `<strong>Γé╣${current.rate}</strong> per <strong>${current.mode === 'MONTHLY' ? 'Month' : 'Day'}</strong>`;
        }
        ...
        tbody.innerHTML = history.map((item, index) => `
            <tr>
                ...
                <td style="text-align:right;">
                    <button onclick="SettingsModule.deletePenaltyEntry('${item.effDate}')" style="background:none; border:none; cursor:pointer; font-size:1.1rem;" title="Delete">≡ƒùæ∩╕Å</button>
                </td>
            </tr>
        `).join('');
    },
    ...
    bindDataEvents() {
        ...
        if (btnCloud) {
            btnCloud.addEventListener('click', async () => {
                ...
                try {
                    btn.disabled = true;
                    btn.innerHTML = '<span>ΓÅ│</span> Syncing...';
    ...
    renderGstList() {
        ...
        tbody.innerHTML = this.gstHistory.map((item, index) => `
            <tr>
                <td>${item.date}</td>
                <td>${item.rate}%</td>
                <td><button onclick="SettingsModule.removeGstEntry(${index})" style="color:red; background:none; border:none; cursor:pointer;">├ù</button></td>
            </tr>
        `).join('');
    },
                ...
                rows += `
                    <tr style="${isSkipped ? 'opacity: 0.5; background: #f8fafc;' : ''} ${noEmail ? 'background: #fff7ed;' : ''}">
                        ...
                        <td style="padding: 10px;">
                            ...
                            ${noEmail ? '<span style="color:#f97316; font-size:0.7rem; display:block;">ΓÜá∩╕Å No Email</span>' : ''}
                        </td>
                        <td style="padding: 10px;">Γé╣${c.dues.totalAmount.toFixed(0)}</td>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn-history-notice" title="Communication History" 
                                ...
                                data-shop="${app.shopNo}">≡ƒô£</button>
                        </td>
                        <td>${app.applicantName}</td>
                        <td style="text-align: center; font-weight: bold;">${monthsDisplay}</td>
                        <td>Γé╣${dues.baseRent.toFixed(2)}</td>
                        <td>Γé╣${dues.gst.toFixed(2)}</td>
                        <td style="color: #ef4444;">Γé╣${dues.penalty.toFixed(2)}</td>
                        <td style="font-weight: bold;">Γé╣${dues.totalAmount.toFixed(2)}</td>
                        <td style="display: flex; gap: 5px;">
                            ...
                            <button class="btn-email-notice btn-primary" 
                                ...
                                ${esc.tooRecent ? 'Sent Γ£ë∩╕Å' : 'Γ£ë∩╕Å Email'}
                            </button>
                        </td>
    ...
    async showHistoryTimeline(shopNo) {
        ...
        overlay.innerHTML = `
            <div class="glass-panel" style="width: 100%; max-width: 400px; padding: 25px; position: relative; background: #fff;">
                ...
                <h3 style="margin-bottom: 20px; border-bottom: 2px solid #f1f5f9; padding-bottom: 10px;">≡ƒôï History: Shop ${shopNo}</h3>
                ...
                <div style="padding: 25px; line-height: 1.6;">
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; background: #fffcf0; border: 1px solid #fde68a; border-radius: 6px;">
                        <tr>
                            ...
                            <td style="padding: 12px; border-bottom: 1px solid #fef3c7; text-align: right;">Γé╣${(dues.baseRent + dues.gst).toFixed(2)}</td>
                        </tr>
                        <tr>
                            <td style="padding: 12px; border-bottom: 1px solid #fef3c7;">Penalty Amount:</td>
                            <td style="padding: 12px; border-bottom: 1px solid #fef3c7; text-align: right;">Γé╣${dues.penalty.toFixed(2)}</td>
                        </tr>
                        <tr style="font-weight: bold; color: #b91c1c; font-size: 16px;">
                            <td style="padding: 12px;">Total Outstanding:</td>
                            <td style="padding: 12px; text-align: right;">Γé╣${dues.totalAmount.toFixed(2)}</td>
                        </tr>
                    </table>
        ...
        try {
            ...
            const noticeBody = `
                ...
                <div style="display: flex; align-items: flex-start; margin-bottom: 0.5rem; margin-left: 2.5rem; font-size: 12pt; line-height: 1.5;">
                    ...
                    <div style="text-align: justify; line-height: 1.5;">
                         SUDA Siddipet ΓÇô Letting out of SUDA Shop No. <strong>${app.shopNo}</strong>, in favor of <strong>${app.applicantName}</strong> 
                        being the highest bidder for <u>Rs. ${app.rentTotal}</u> per month plus GST@18% w.e.f 
                        ${new Date(app.rentStartDate || app.leaseDate).toLocaleString('default', { month: 'long', year: 'numeric' })} ΓÇô 
                        ...
                        take action to evict from the Shop ΓÇô notice issued.
                    </div>
                </div>
            ...
            overlay.innerHTML = `
                ...
                <div class="toolbar">
                    ...
                    <button id="btn-email-action" class="btn-preview" style="background: #4f46e5;">Γ£ë∩╕Å Email Notice</button>
                    <button id="btn-print-action" class="btn-preview" style="background: #0ea5e9;">≡ƒôæ Download PDF</button>
                    <button id="btn-physical-action" class="btn-preview" style="background: #10b981;">≡ƒû¿∩╕Å Record Physical Service</button>
                    <div style="color: white; font-size: 0.8rem; margin: auto 10px;">Γ£ì∩╕Å <i>Click text to edit</i></div>
                </div>
            ...
            if (esc.tooRecent) {
                ...
                physicalBtn.textContent = 'Recorded ≡ƒû¿∩╕Å';
            }
            ...
            physicalBtn.onclick = async () => {
                if (confirm("Have you served this notice physically? This will record the action in history.")) {
                    ...
                    if (ok) {
                        ...
                        physicalBtn.textContent = '≡ƒû¿∩╕Å Record Physical Service';
                    }
                }
            };
            ...
            if (esc.tooRecent) {
                ...
                emailActionBtn.textContent = 'Sent Γ£ë∩╕Å';
            }
...
const GstRemittanceModule = {
            ...
            <div class="glass-panel">
                ...
                <div style="display: flex; gap: 1rem; margin-top: 1.5rem; justify-content: space-between;">
                    <div class="glass-panel" style="background: rgba(255,255,255,0.6); flex:1; text-align: center;">
                        ...
                        <div id="gst-collected" style="font-size: 1.8rem; font-weight: bold; color: #6366f1;">Γé╣0.00</div>
                    </div>
                    <div class="glass-panel" style="background: rgba(255,255,255,0.6); flex:1; text-align: center;">
                        <h4 style="color: var(--text-muted); font-size: 0.9rem;">Total Remitted</h4>
                        <div id="gst-remitted" style="font-size: 1.8rem; font-weight: bold; color: #10b981;">Γé╣0.00</div>
                    </div>
                    <div class="glass-panel" style="background: rgba(255,255,255,0.6); flex:1; text-align: center;">
                        <h4 style="color: var(--text-muted); font-size: 0.9rem;">Pending Remittance</h4>
                        <div id="gst-pending" style="font-size: 1.8rem; font-weight: bold; color: #ef4444;">Γé╣0.00</div>
                    </div>
                </div>
                ...
                <div class="glass-panel" style="margin-top: 2rem; border: 1px solid #e2e8f0;">
                    ...
                    <form id="remittance-form" style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 1rem; align-items: end;">
                        ...
                        <div class="form-group">
                            <label class="form-label">Amount (Γé╣)</label>
    ...
    getStats(month = '', year = '') {
        ...
        if (totalRemitted > totalCollected) {
            console.warn(`GST Remittance: remitted (Γé╣${totalRemitted.toFixed(2)}) > collected (Γé╣${totalCollected.toFixed(2)}) for filter month=${month} year=${year}`);
        }
    ...
    updateStats(month = '', year = '') {
        ...
        document.getElementById('gst-collected').textContent = `Γé╣${stats.totalCollected.toFixed(2)}`;
        document.getElementById('gst-remitted').textContent = `Γé╣${stats.totalRemitted.toFixed(2)}`;
        document.getElementById('gst-pending').textContent = `Γé╣${stats.pending.toFixed(2)}`;
        ...
        try {
            ...
            if (month && year) {
                ...
                labelText += `${monthNames[mIdx] || month} ΓÇö FY ${year}-${String(parseInt(year) + 1).slice(2)}`;
            } else if (year) {
            ...
            try {
                const countsPart = ` ΓÇö payments: ${stats.matchedPaymentsCount || 0}, remittances: ${stats.matchedRemittancesCount || 0}`;
            ...
            if (warnEl) {
                if (stats.totalRemitted > stats.totalCollected) {
                    ...
                    warnEl.style.display = 'inline';
                    warnEl.textContent = `Warning: Remitted exceeds collected by Γé╣${diff} for the selected period.`;
                    ...
                    warnEl.textContent = `Shortfall: You need to remit Γé╣${diff} more.`;
                } else {
    ...
    renderHistory(month = '', year = '') {
        ...
        tbody.innerHTML = remittances.map(r => {
            ...
            <tr>
                ...
                <td style="font-weight: bold; color: #10b981;">Γé╣${Utils.parseNumber(r.amount).toFixed(2)}</td>
            </tr>
            `;
        }).join('');
    },
    ...
    renderMonthlySummary(year = '') {
        ...
        const rows = order.map(m => {
            ...
            const tolerance = 1.0; // Γé╣1 differrence allowed for rounding
            ...
            return `
                <tr data-month="${m}" style="cursor: pointer;">
                    ...
                    <td>Γé╣${data.collected.toFixed(2)}</td>
                    <td>Γé╣${data.remitted.toFixed(2)}</td>
                    <td style="color: ${pending < 0 ? '#ef4444' : '#059669'};">Γé╣${pending.toFixed(2)}</td>
                    <td>${statusBadge}</td>
                </tr>
            `;
        }).filter(r => r !== '');
...
const LeaseStatusModule = {
                ...
                <div class="glass-panel" style="background: white; width: 500px; max-width: 90%; max-height: 90vh; overflow-y: auto;">
                    ...
                    <form id="renewal-form">
                        ...
                        <div id="rent-update-container" style="display: none; background: #f1f5f9; padding: 1rem; border-radius: 6px; margin-top: 0.5rem;">
                            <div class="form-group">
                                <label class="form-label">New Base Rent (Γé╣)</label>
                            ...
                            <div id="prev-base-rent" style="display:none; margin-bottom:8px; font-size:0.9rem; color:#475569;">
                                <strong>Previous Base Rent:</strong> <span id="prev-base-rent-value" style="font-weight:700;">Γé╣0.00</span>
                            </div>
    ...
    bindEvents() {
        ...
        inputBase.addEventListener('input', (e) => {
            ...
            displayTotal.textContent = `Γé╣${base.toFixed(2)} + Γé╣${gst.toFixed(2)} (GST) = Γé╣${total.toFixed(2)}`;
        });
    ...
    openRenewalModal(shopNo) {
        ...
        if (prevValueEl) prevValueEl.textContent = `Γé╣${(parseFloat(app.rentBase) || 0).toFixed(2)}`;
...
const ReportModule = {
        ...
        window.debugShopPenalty = (shopNo) => {
            ...
            window.dcbDebugLog.forEach(row => {
                if (row.Penalty > 0) {
                    ...
                    msg += `${row.Month}: ${row.Days} ${unitLabel} overdue -> Γé╣${row.Penalty}\n`;
                }
            });
    ...
    generateDCB() {
        ...
        targets.forEach(app => {
            ...
            html += `
                <tr>
                    ...
                    <td>${app.shopNo}</td>
                    <td>${displayName}</td>
                    <td>Γé╣${result.currentDemandBase.toFixed(2)}</td>
                    <td>Γé╣${result.currentDemandGst.toFixed(2)}</td>
                    <td>Γé╣${result.arrearDemandBase.toFixed(2)}</td>
                    <td>Γé╣${result.arrearDemandGst.toFixed(2)}</td>
                    <td>Γé╣${result.arrearDemandPenalty.toFixed(2)}</td>
                    <td>Γé╣${result.totalDemand.toFixed(2)}</td>
                    <td>Γé╣${(result.currentCollection - (result.currentCollectionPenalty || 0)).toFixed(2)}</td>
                    <td>Γé╣${(result.currentCollectionPenalty || 0).toFixed(2)}</td>
                    <td>Γé╣${result.arrearCollection.toFixed(2)}</td>
                    <td>Γé╣${result.totalCollection.toFixed(2)}</td>
                    <td>Γé╣${result.currentBalance.toFixed(2)}</td>
                    <td>Γé╣${result.arrearBalance.toFixed(2)}</td>
                    <td style="color: ${result.totalBalance > 0 ? '#ef4444' : '#10b981'}; font-weight: bold;">Γé╣${result.totalBalance.toFixed(2)}</td>
                    <td>${pct}%</td>
        ...
        document.getElementById('dcb-foot').innerHTML = `
            <tr>
                ...
                <td>Γé╣${totalCurrentDemandBase.toFixed(2)}</td>
                <td>Γé╣${totalCurrentDemandGst.toFixed(2)}</td>
                <td>Γé╣${totalArrearDemandBase.toFixed(2)}</td>
                <td>Γé╣${totalArrearDemandGst.toFixed(2)}</td>
                <td>Γé╣${totalArrearDemandPenalty.toFixed(2)}</td>
                <td>Γé╣${totalTotalDemand.toFixed(2)}</td>
                <td>Γé╣${(totalCurrentCollection - totalCurrentCollectionPenalty).toFixed(2)}</td>
                <td>Γé╣${totalCurrentCollectionPenalty.toFixed(2)}</td>
                <td>Γé╣${totalArrearCollection.toFixed(2)}</td>
                <td>Γé╣${totalTotalCollection.toFixed(2)}</td>
                <td>Γé╣${totalCurrentBalance.toFixed(2)}</td>
                <td>Γé╣${totalArrearBalance.toFixed(2)}</td>
                <td>Γé╣${totalTotalBalance.toFixed(2)}</td>
    ...
    renderStatement(container) {
        ...
        const penaltyText = m === 'MONTHLY' ? `Γé╣${r}/month` : `Γé╣${r}/day`;
        ...
        if (!dues.details || dues.details.length === 0) {
            ...
            dues.details.forEach(d => {
                ...
                html += `
                    <tr>
                        ...
                        <td style="text-align: right; padding: 8px;">Γé╣${d.rent.toFixed(2)}</td>
                        <td style="text-align: right; padding: 8px; color: #ef4444;">Γé╣${d.penalty.toFixed(2)}</td>
                        <td style="text-align: right; padding: 8px; font-weight: 500;">Γé╣${total.toFixed(2)}</td>
                    </tr>
                `;
            });
            ...
            tfoot.innerHTML = `
                <tr>
                    ...
                    <td style="text-align: right; padding: 10px;">Γé╣${(dues.baseRent + dues.gst).toFixed(2)}</td>
                    <td style="text-align: right; padding: 10px;">Γé╣${dues.penalty.toFixed(2)}</td>
                    <td style="text-align: right; padding: 10px; font-size: 1.1rem;">Γé╣${dues.totalAmount.toFixed(2)}</td>
                </tr>
            `;
        }
        ...
        const rowsHtml = results.rows.map((r, idx) => {
            ...
            return `
                <tr>
                    ...
                    <td class="text-right">Γé╣${res.currentDemandBase.toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.currentDemandGst.toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.arrearDemandBase.toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.arrearDemandGst.toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.arrearDemandPenalty.toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.totalDemand.toFixed(2)}</td>
                    <td class="text-right">Γé╣${(res.currentCollection - (res.currentCollectionPenalty || 0)).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(res.currentCollectionPenalty || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.arrearCollection.toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.totalCollection.toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.currentBalance.toFixed(2)}</td>
                    <td class="text-right">Γé╣${res.arrearBalance.toFixed(2)}</td>
                    <td class="text-right font-bold">Γé╣${res.totalBalance.toFixed(2)}</td>
        ...
        const totalsHtml = `
            <tfoot>
                <tr style="font-weight:700; background:#f1f5f9;">
                    ...
                    <td class="text-right">Γé╣${(totals.totalCurrentDemandBase || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalCurrentDemandGst || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalArrearDemandBase || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalArrearDemandGst || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalArrearDemandPenalty || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalTotalDemand || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${((totals.totalCurrentCollection || 0) - (totals.totalCurrentCollectionPenalty || 0)).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalCurrentCollectionPenalty || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalArrearCollection || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalTotalCollection || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalCurrentBalance || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalArrearBalance || 0).toFixed(2)}</td>
                    <td class="text-right">Γé╣${(totals.totalTotalBalance || 0).toFixed(2)}</td>
        ...
        if (dues.details && dues.details.length > 0) {
            dues.details.forEach((m, idx) => {
                ...
                <tr>
                    ...
                    <td style="border: 1px solid #e2e8f0; padding: 8px; text-align: right;">Γé╣${m.rent.toFixed(2)}</td>
                    <td style="border: 1px solid #e2e8f0; padding: 8px; text-align: right;">Γé╣${(m.gst || 0).toFixed(2)}</td>
                    <td style="border: 1px solid #e2e8f0; padding: 8px; text-align: right;">Γé╣${m.penalty.toFixed(2)}</td>
                    <td style="border: 1px solid #e2e8f0; padding: 8px; text-align: right; font-weight: bold;">Γé╣${(m.rent + (m.gst || 0) + m.penalty).toFixed(2)}</td>
                </tr>
                `;
            });
            rows += `
            <tr style="background: #f8fafc; font-weight: bold;">
                ...
                <td style="border: 1px solid #e2e8f0; padding: 10px; text-align: right;">Γé╣${dues.baseRent.toFixed(2)}</td>
                <td style="border: 1px solid #e2e8f0; padding: 10px; text-align: right;">Γé╣${dues.gst.toFixed(2)}</td>
                <td style="border: 1px solid #e2e8f0; padding: 10px; text-align: right;">Γé╣${dues.penalty.toFixed(2)}</td>
                <td style="border: 1px solid #e2e8f0; padding: 10px; text-align: right;">Γé╣${dues.totalAmount.toFixed(2)}</td>
            </tr>
            `;
        } else {
            ...
            try {
                ...
                if (!error && data && rpcSummaryDiv) {
                    rpcSummaryDiv.innerHTML = `
                        <strong>Total Base Rent Paid:</strong> Γé╣${parseFloat(data.rent_paid || 0).toLocaleString('en-IN')}<br>
        ...
        if (dues.details && dues.details.length > 0) {
            dues.details.forEach((m, idx) => {
                ...
                <tr>
                    ...
                    <td class="data-cell" style="text-align: right; padding: 8px;">Γé╣${m.rent.toFixed(2)}</td>
                    <td class="data-cell" style="text-align: right; padding: 8px;">Γé╣${(m.gst || 0).toFixed(2)}</td>
                    <td class="data-cell" style="text-align: right; padding: 8px;">Γé╣${m.penalty.toFixed(2)}</td>
                    <td class="data-cell" style="text-align: right; padding: 8px; font-weight: bold;">Γé╣${(m.rent + (m.gst || 0) + m.penalty).toFixed(2)}</td>
                </tr>
            `;
            });
            html += `
                <tr style="border-top: 2px solid #000; font-weight: bold; font-size: 1.05rem;">
                    <td colspan="2" style="text-align: right; padding: 10px;">TOTAL OUTSTANDING</td>
                    <td style="text-align: right; padding: 10px;">Γé╣${dues.baseRent.toFixed(2)}</td>
                    <td style="text-align: right; padding: 10px;">Γé╣${dues.gst.toFixed(2)}</td>
                    <td style="text-align: right; padding: 10px;">Γé╣${dues.penalty.toFixed(2)}</td>
                    <td style="text-align: right; padding: 10px; font-size: 1.1rem;">Γé╣${dues.totalAmount.toFixed(2)}</td>
                </tr>
            `;
            ...
            <div class="glass-panel">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
                    ...
                    <div style="display: flex; gap: 0.5rem;">
                        <button class="btn-primary" id="btn-export-gst-excel" style="background: #059669;">≡ƒôè Export to Excel</button>
                        <button class="btn-primary" id="btn-export-gst-pdf" style="background: #2563eb;">≡ƒôä Export to PDF</button>
                    </div>
                </div>
                    ...
                    <table class="data-table" id="gst-monthwise-table" style="border-collapse: collapse; width: 100%; font-size: 0.9rem;">
                        ...
                        <tfoot id="gst-monthwise-footer" style="font-weight: bold; background: #f1f5f9; border: 1px solid #cbd5e1;">
                            <tr>
                                ...
                                <td style="border: 1px solid #cbd5e1; padding: 8px;">Γé╣0.00</td>
                                <td style="border: 1px solid #cbd5e1; padding: 8px;">Γé╣0.00</td>
                                <td style="border: 1px solid #cbd5e1; padding: 8px;">Γé╣0.00</td>
                                <td style="border: 1px solid #cbd5e1; padding: 8px;">Γé╣0.00</td>
                            </tr>
                ...
                <div id="gst-form58-action" style="margin-top: 1rem; text-align: left; display: none; padding-top: 1rem; border-top: 1px dashed #cbd5e1;">
                    <button class="btn-primary" id="btn-gen-form58" style="background: #4f46e5; display: flex; align-items: center; gap: 0.5rem;">
                         <span style="font-size: 1.2rem;">≡ƒôä</span> Generate Form-58
                    </button>
        ...
        tbody.innerHTML = filtered.map(p => {
            ...
            return `
                <tr style="border: 1px solid #cbd5e1;">
                    ...
                    <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: right;">Γé╣${rent.toFixed(2)}</td>
                    <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: right;">Γé╣${gst.toFixed(2)}</td>
                    <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: right;">Γé╣${penalty.toFixed(2)}</td>
                    <td style="border: 1px solid #cbd5e1; padding: 8px; text-align: right; font-weight: bold;">Γé╣${total.toFixed(2)}</td>
                </tr>
    `;
        }).join('');
    ...
    updateFooter(tfoot, rent, gst, penalty, total) {
        tfoot.innerHTML = `
            <tr style="border: 1px solid #cbd5e1;">
                ...
                <td style="border: 1px solid #cbd5e1; padding: 8px; font-weight: bold;">Γé╣${rent.toFixed(2)}</td>
                <td style="border: 1px solid #cbd5e1; padding: 8px; font-weight: bold;">Γé╣${gst.toFixed(2)}</td>
                <td style="border: 1px solid #cbd5e1; padding: 8px; font-weight: bold;">Γé╣${penalty.toFixed(2)}</td>
                <td style="border: 1px solid #cbd5e1; padding: 8px; font-weight: bold;">Γé╣${total.toFixed(2)}</td>
            </tr>
    `;
    },
            ...
            <div class="page-box" style="margin-top: 70px;">
                ...
                <!-- Amounts -->
                 <div style="display: flex; border: 2px solid #000; border-top: 0; padding: 5px; font-weight: bold; font-size: 11pt; justify-content: space-around;">
                    <div>Gross &nbsp;&nbsp; Γé╣ &nbsp;&nbsp; ${totalGST.toFixed(2)}</div>
                    <div>Deduction &nbsp;&nbsp; Γé╣ &nbsp;&nbsp; 0.00</div>
                    <div>Net &nbsp;&nbsp; Γé╣ &nbsp;&nbsp; ${totalGST.toFixed(2)}</div>
                </div>
                ...
                <div class="body-text-section">
                     ...
                     <div style="margin-top: 10px; margin-left: 40%; font-size: 9pt;">
                        1. Γé╣....................... by transfer credit to the S.B.<br>
                        Accounts of the employee (As per Annexure - 1)<br><br>
                        
                        2. Γé╣....................... by transfer credit to the D.D.O. Account<br>
                 ...
                 <table class="bordered" style="width: 100%; height: 400px;">
                    ...
                    <tbody>
                        ...
                        <tr style="height: 30px;">
                            ...
                            <td style="text-align: center; font-weight: bold;">Γé╣ ${totalGST.toFixed(2)}</td>
                        </tr>
                    </tbody>
                </table>
        ...
        const content = `
            <div style="padding: 2rem; max-width: 800px; margin: 0 auto; border: 2px solid #333; position: relative;">
                ...
                <div style="font-size: 1.1rem; line-height: 1.6;">
                    ...
                    <p>A sum of Rupees <strong>${amountWords} Only</strong> (Γé╣${amount})</p>
            ...
            tbody.innerHTML = payments.map(p => {
                ...
                return `
                    <tr>
                        ...
                        <td>
                            <button class="btn-delete-pay" data-ts="${p.timestamp}" style="background:none; border:none; cursor:pointer;" title="Delete Payment">Γ¥î</button>
                        </td>
                    </tr>
                `;
            }).join('');
    ...
    renderHistory() {
        ...
        tbody.innerHTML = waivers.map(w => `
            <tr>
                ...
                <td>
                    <button class="btn-delete-waiver" data-id="${w.id}" style="color:red;border:none;background:none;cursor:pointer;">≡ƒùæ∩╕Å</button>
                </td>
            </tr>
        `).join('');
